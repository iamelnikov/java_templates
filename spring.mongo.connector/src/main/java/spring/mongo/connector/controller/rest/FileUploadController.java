...
public class FileUploadController {
	...
	public @ResponseBody String postUploadVideo(
			@RequestPart("fileMeta") FileMeta wrapper,
		...
		if (file != null) {
			try {
				...
				String oid = fileDao.storeMediaFile(istr, fileName, cType,
						wrapper);
				return "File Successfully Uploaded " + oid;
			} catch (Exception e) {
				e.printStackTrace();
				return e.getMessage();
			}
	...
	@RequestMapping(value = "/spring.mongo/v1/uploadFile2", method = RequestMethod.POST)
	public @ResponseBody String postUploadVideo2(
		...
		if (file != null) {
			try {
				...
				String fileName = file.getName();
				String cType = file.getContentType();
				FileMeta meta = new FileMeta(un, fn);
				String id = fileDao.storeMediaFile(istr, fileName, cType, meta);
				return "File Successfully Uploaded " + id;
			} catch (Exception e) {
				e.printStackTrace();
				return e.getMessage();
			}
	...
	@RequestMapping(value = "/spring.mongo/v1/uploadFile3", method = RequestMethod.POST)
	public @ResponseBody String postUploadVideo3(
			@RequestPart("fileMeta") FileMeta meta,
			@RequestPart("file") MultipartFile is) {
		if (is != null) {
			try {
				InputStream istr = is.getInputStream();
				String fileName = is.getName();
				String cType = is.getContentType();
				fileDao.storeMediaFile(istr, fileName, cType, meta);
	...
	@RequestMapping(value = "/spring.mongo/v1/getFile/{id}", method = RequestMethod.GET)
	public void getFile(@PathVariable("id") String id,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		id = "55856d645e0a7a5e99412ade";
		serveResource(id, request, response);
	}
	
	private static Logger logger = Logger.getAnonymousLogger();

	public void serveResource(String id, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		if (response == null || request == null) {
			return;
		}
		GridFSDBFile file = fileDao.getVideoById(id);
		...
		if (range != null) {
			...
			if (ranges.isEmpty()) {
				for (String part : range.substring(6).split(",")) {
					...
					long end = Range.sublong(part, part.indexOf("-") + 1,
							part.length());
		...
		if (contentType == null) {
			...
			disposition = accept != null
					&& HttpUtils.accepts(accept, contentType) ? "inline"
					: "attachment";
		}
		// Initialize response.
		response.reset();
		...
		try (InputStream input = new BufferedInputStream(file.getInputStream());
			...
			if (ranges.isEmpty() || ranges.get(0) == full) {
				...
				response.setHeader("Content-Range", "bytes " + full.start + "-"
						+ full.end + "/" + full.total);
